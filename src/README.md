...
In this lab, you do not need to make requests in the same cycle `rdy` goes high. You **do**, however, need to make sure you deassert `rdy` in the cycle following `en`.
...
In this task, you will get started by creating a RAM block using the Megafunction Wizard, creating circuitry to fill the memory, and observing the contents using the In-System Memory Content Editor.
...
In your testbench, you will likely want to use `$readmemh()` to initialize memories and compare them to a known reference state. You can look up how `$readmemh()` works in the _SystemVerilog 2017 Language Standard_ document on Piazza. If you read any external files in your testbench, you will have to commit them **in the same folder** as the testbench that uses them, as the autograder will not try to guess where you might have put them.
